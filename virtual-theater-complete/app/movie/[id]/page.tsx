  ...
  return (
    <div className="grid grid-featured">
      <div className="panel">
        ...
        <div className="small">
          Mux Playback: {movie.muxPlaybackId ? "Ready" : "Not ready"}
        </div>
      </div>
      <div>
        <h1>{movie.title}</h1>
        ...
        <div className="flex-wrap-gap-12">
          <form action={`/api/checkout`} method="POST">
            ...
            <button className="btn gold" type="submit">
              Rent 48 hrs ${(movie.priceRent / 100).toFixed(2)}
            </button>
          </form>
          <form action={`/api/checkout`} method="POST">
            <input type="hidden" name="movieId" value={movie.id} />
            ...
            <button className="btn" type="submit">
              Buy ${(movie.priceBuy / 100).toFixed(2)}
            </button>
          </form>
          {movie.trailerMuxPlaybackId && (
            <a className="btn" href={`/watch/${movie.id}?preview=1`}>
              Play Trailer
            </a>
          )}
          {movie.muxPlaybackId && (
            <a className="btn" href={`/watch/${movie.id}`}>
              Watch
            </a>
          )}
          {isLoggedIn && (
