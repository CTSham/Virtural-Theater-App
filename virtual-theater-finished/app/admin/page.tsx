...
export default async function Admin() {
  ...
  const user = me
    ? await prisma.user.findUnique({ where: { email: me } })
    : null;
  ...
  const chartBars = Object.entries(byMovie)
    .map(([title, count], i) => {
      const height = Math.min(120, count * 10);
      const x = 20 + i * 60;
      const y = 140 - height;
      return (
        `<rect x="${x}" y="${y}" width="40" height="${height}" rx="6" ry="6" />` +
        `<text x="${
          x + 20
        }" y="160" font-size="10" text-anchor="middle">${title.slice(
          0,
          8
        )}</text>` +
        `<text x="${x + 20}" y="${
  ...
  return (
    <div className="grid grid-admin">
          ...
          <tbody>
            {movies.map((m: any) => (
              <tr key={m.id}>
                ...
                <td>
                  ${(m.priceRent / 100).toFixed(2)} / $
                  {(m.priceBuy / 100).toFixed(2)}
                </td>
                <td>
                  <Link href={`/admin/set-playback/${m.id}`}>Edit</Link>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="panel">
        <h2>Analytics</h2>
        <div className="small">
          Revenue: ${(revenue / 100).toFixed(2)} • Tickets: {purchases.length}
        </div>
        <svg
          width="400"
          height="180"
          ...
          dangerouslySetInnerHTML={{
            __html: `<g fill='#9be9df'>${chartBars}</g>`,
          }}
        />
